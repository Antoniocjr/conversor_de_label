    var autoClean = true, outputValue, outputValue2, outputValue3, lastInputId, operadora, label, formatedGCLabel, isSpecial = true;
    ...
    { 
		...
		{id:55, nome:"EUCARD", cod:810, layout:""},
		{id:56, nome:"TRICARD", cod:440, layout:""}]
    };
...
function Clean() 
    {
        ...
        if(document.getElementById("inputOutput1") != null)
        {
			...
			document.getElementById("inputOutput2").parentNode.removeChild(document.getElementById("inputOutput2"));
        }
    }
...
function Verify() 
{
    ...
    { 
		...
		{id:55, layout: "EUC_" + label + "_%Y%m%d_&C(6).TXT"},
		{id:56, layout: "TRICARD_" + label + "_%Y-%m-%d_venda.ret", layout2:"TRICARD_"+ label +"_%Y-%m-%d_financeiro.ret", layout3:"TRICARD_"+ label +"_%Y-%m-%d_saldo.ret"},]
    };
    ...
    if(label != undefined && operadora != undefined)
    {
        if(Ndata.operadoras.find(operadoras => operadoras.nome === operadora))
        {
            ...
            {
            isSpecial = false;
		    if(document.getElementById("inputOutput2") != null)
            {
					document.getElementById("inputOutput2").parentNode.removeChild(document.getElementById("inputOutput2"));
            }
            ...
            newOuput1.setAttribute("size", "20");
            newOuput1.setAttribute("readonly", "true");
			var newOuput2 = document.createElement('input');
            var referenceNode1 = document.getElementById("inputOutput");
            newOuput2.setAttribute("id", "inputOutput2");
            newOuput2.setAttribute("class", "w3-input w3-center w3-animate-right w3-border-0");
            newOuput2.setAttribute("type", "text");
            newOuput2.setAttribute("name", "output2");
            newOuput2.setAttribute("size", "20");
            newOuput2.setAttribute("readonly", "true");

            idActualOperadora = Ndata.operadoras.find(operadoras => operadoras.nome === operadora).id;
            outputValue = Ldata.operadoras.find(operadoras => operadoras.id === idActualOperadora).layout;  
            outputValue2 = Ldata.operadoras.find(operadoras => operadoras.id === idActualOperadora).layout2;    
            outputValue3 = Ldata.operadoras.find(operadoras => operadoras.id === idActualOperadora).layout3;
			referenceNode.parentNode.insertBefore(newOuput1, referenceNode.nextElementSibling);
            referenceNode1.parentNode.insertBefore(newOuput2, referenceNode1.nextElementSibling);
            updateSpecialTRIDIV();
            }else
            {
            idActualOperadora = Ndata.operadoras.find(operadoras => operadoras.nome === operadora).id;
    ...
    function updateSpecialTRIDIV()
    {
        document.convert.output.value = outputValue;
        document.convert.output1.value = outputValue2;
		document.convert.output2.value = outputValue3;
        document.getElementById("tempOutput").innerHTML = operadora + " - " + Ndata.operadoras.find(operadoras => operadoras.nome === operadora).cod; 
    }
}
